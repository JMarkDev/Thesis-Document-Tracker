  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-2xl p-5 bg-white rounded-lg shadow-lg">
        ...
        {isReceived && !isForwarded && !isReturned && !officeUploader && (
          <div className="w-full">
            <label>
              <strong className="text-gray-800">Comments:(Optional)</strong>
            </label>
            <textarea
              name=""
              rows={4}
        ...
        {isRecipient && (
          <div className="flex justify-end items-center pt-3 border-t">
            {isForwarded ? (
              ...
              <p className="text-gray-600">
                {isReceived && !isForwarded && !officeUploader && (
                  <button
              ...
              <div className="flex gap-3">
                ...
                {/* {isReturned && alreadyReturn ? (
                  <p className="text-gray-600">
                    Document has been returned to the previuos office.
                  </p>
                ) : (
                  <button
                    className="px-6 py-2 text-white bg-green-600 hover:bg-green-800 rounded-md shadow-md transition"
                    onClick={() =>
                      handleReceive(isReceived ? "forwarded" : "received")
                    }
                  >
                ...
                {/* Check if isReturned and alreadyReturn */}
                {isReturned && alreadyReturn ? (
                  <p className="text-gray-600">
                    Document has been returned to the previous office.
                  </p>
                ) : (
                  // Only show the Forward/Receive button when NOT isReturned && !alreadyReturn
                  !isReturned && (
                    <button
                      className="px-6 py-2 text-white bg-green-600 hover:bg-green-800 rounded-md shadow-md transition"
                      onClick={() =>
                ...
                {/* Show the Return button when isReturned is true and alreadyReturn is false */}
                {isReturned && !alreadyReturn && (
                  <button
                    className="px-6 py-2 text-white bg-red-500 hover:bg-red-800 rounded-md shadow-md transition"
                    onClick={() => handleReceive("returned")}
...
ReceiveDocument.propTypes = {
  ...
  officeUploader: PropTypes.bool,
  isReturned: PropTypes.bool,
  alreadyReturn: PropTypes.bool,
};
