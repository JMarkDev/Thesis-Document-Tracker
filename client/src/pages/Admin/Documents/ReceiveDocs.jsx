...
// import ReceiveDocument from "../EsuCampusScan/ReceivedDocument";
...
import { useState, useEffect } from "react";
import ReceiveDocument from "../ScanNow/ReceiveDocument.jsx";
import { useDispatch, useSelector } from "react-redux";
import {
  fetchDocumentByTrackingNum,
  getDocumentByTrackingNumber,
...
import documentStatus from "../../../constants/documentStatusList.js";
import PropTypes from "prop-types";
...
const ScanNow = ({ trackingNumber }) => {
  ...
  const [campusUploader, setCampusUploader] = useState();
  ...
  useEffect(() => {
    if (user?.role === rolesList.campus_admin) {
      setCampus(`${user?.esuCampus} CAMPUS ADMIN`);
    } else {
      setCampus(`${user?.esuCampus} REGISTRAR`);
    }
  ...
  //   let campus;
  //   if (user?.role === rolesList.campus_admin) {
  //     campus = `${user?.esuCampus} CAMPUS ADMIN`;
  //   } else if (user?.role === rolesList.registrar) {
  //     campus = `${user?.esuCampus} REGISTAR`;
  //   }

  //   const checkUploader =
  //     document?.document_recipients[0]?.office_name === campus;
  //   console.log(checkUploader);
  //   if (checkUploader) {
  //     setOfficeUploader(true);
  //   }
  // }, [user, document, officeUploader]);
  ...
  useEffect(() => {
    ...
    if (user?.role === rolesList.campus_admin) {
      ...
      campus = `${user?.esuCampus} REGISTRAR`;
    }

    const isUploader = document?.document_recipients[0]?.office_name === campus;

    setOfficeUploader(isUploader);
  ...
  const handleClose = () => {
    ...
    setDocumentData(null); // Clear document data
    setLoading(false); // Reset loading state
    setIsReturned(false); // Reset return state
    setOfficeUploader(false); // Reset uploader state
    setIsReceived(false);
    setCampusUploader(false);
    setIsForwarded(false);
    setIsReturned(false);

    // Dispatch actions to reset Redux state if needed
    dispatch(
      fetchDocumentByTrackingNum({
        tracking_number: trackingNumber,
        toast: toastUtils(),
