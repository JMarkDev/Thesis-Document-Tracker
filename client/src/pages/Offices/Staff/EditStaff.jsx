...
import { fetchStaff } from "../../../services/staffSlice";

const EditStaff = ({ modal, closeModal, id, email, officeId }) => {
  const dispatch = useDispatch();
  const [showPass, setShowPass] = useState(false);
  const toast = useToast();
  const { register, handleSubmit, setValue } = useForm();
  const [loading, setLoading] = useState(false);
  const office = useSelector(getFetchedUserById);
  ...
  const onSubmit = async (data) => {
    data.role = rolesList.office_staff;
    setLoading(true);
    ...
    try {
      ...
      const response = await api.put(`/office/update-staff/${email}`, formData);
      if (response.data.status === "success") {
        toast.success(response.data.message);
        closeModal(false);
        setLoading(false);
        dispatch(fetchStaff(officeId));
      }
    } catch (error) {
      console.log(error);
      setLoading(false);
              ...
              <div className="p-4  space-y-4 text-sm text-[#221f1f]">
                ...
                >
                  <div className="flex justify-center items-center">
                    <Profile setValue={setValue} image={office?.image} />
                  </div>
